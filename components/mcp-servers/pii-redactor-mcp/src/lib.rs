...
//!
//! This server is **pure compute** — its single tool (`redact`) never performs
//! outbound HTTP, so it never calls [`bridge::outbound`]. The [`bridge`] is
//! still required to drive the MCP streamable-http server transport itself. The
//! workload ships with an empty `allowedHosts` (deny-all): the text this
//! component sees can never leave the sandbox.
//!
...
impl wasip3::exports::http::handler::Guest for Component {
    async fn handle(request: Request) -> Result<Response, ErrorCode> {
        ...
        let service = StreamableHttpService::new(
            || Ok(server::PiiRedactorServer::new()),
            Arc::new(NeverSessionManager::default()),
            config,
        );

        // Run rmcp dispatch until response headers are available. Tool code
        // runs in here; this server's tool is pure compute (no outbound HTTP).
