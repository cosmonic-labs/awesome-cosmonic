...
//!
//! This server is **pure compute** — its tools (`sanitize_html`,
//! `render_markdown`) never perform outbound HTTP, so they never call
//! [`bridge::outbound`]. The [`bridge`] is still required to drive the MCP
//! streamable-http server transport itself. The workload ships with an empty
//! `allowedHosts` (deny-all): the content this component sees can never leave
//! the sandbox.
//!
...
impl wasip3::exports::http::handler::Guest for Component {
    async fn handle(request: Request) -> Result<Response, ErrorCode> {
        ...
        let service = StreamableHttpService::new(
            || Ok(server::SanitizerServer::new()),
            Arc::new(NeverSessionManager::default()),
            config,
        );

        // Run rmcp dispatch until response headers are available. Tool code
        // runs in here; this server's tools are pure compute (no outbound HTTP).
