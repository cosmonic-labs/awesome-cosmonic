...
echo "== driving one message on pull.run"
$NATS pub pull.run "run=$RUN;pad=" || { echo "publish failed"; exit 1; }
sleep 5

echo "== asserting the effect"
$NATS stream get RECEIPTS --last-for "done.js-pull.$RUN" >/dev/null 2>&1 \
  && echo "PASS: receipt on done.js-pull.$RUN" \
