...
echo "== driving one message on (write to the bucket)"
$NATS pub (write to the bucket) "run=$RUN;pad=" || { echo "publish failed"; exit 1; }
sleep 5

echo "== asserting the effect"
$NATS stream get RECEIPTS --last-for "done.kv-watch.$RUN" >/dev/null 2>&1 \
  && echo "PASS: receipt on done.kv-watch.$RUN" \
