...
~53 Mi per additional Go replica against ~4 Mi for Rust - three replicas of
an un-grouped consumer approach the whole budget of a 512Mi host. Prefer a
queue group, and budget memory per replica rather than assuming it is free.
...
`kubectl` shows only `Ready=Unknown`. Check the host log.
