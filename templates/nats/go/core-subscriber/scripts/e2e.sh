...
echo "== driving one message on bench.core"
$NATS pub bench.core "run=$RUN;pad=" || { echo "publish failed"; exit 1; }
sleep 5

echo "== asserting the effect"
$NATS stream get RECEIPTS --last-for "done.core-sink.$RUN" >/dev/null 2>&1 \
  && echo "PASS: receipt on done.core-sink.$RUN" \
