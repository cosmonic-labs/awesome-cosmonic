...
**If you are unsure, start with `jetstream-consumer`.** JetStream paces
delivery by acknowledgement, so a slow consumer is throttled instead of
overrun - the single biggest reliability difference in this interface.
...
| Rust | Go |
|---|---|
| `.cargo/config.toml` - wasip1 target | `Makefile` - componentize-go build |
...
| `wkg.toml` | `docs/building.md`, `docs/limitations.md` |
...
- **`subscription-capacity` is the knob that matters**, and it is denominated
  ...
  same workload. `max-in-flight` bounds how many concurrent instances a
  burst can occupy - bound it on push consumers unless host memory is sized
  for it; [`nats-tuning.md`](nats-tuning.md) §2.1 has the measurements.
- **On Cosmonic Control, grants belong to the operator.** The host plugin's
  `workloadConfig` defaults to `deny` there: declare `subject-allow` /
  `stream-allow` / `bucket-allow` in the hostgroup's
  `hostPlugins: [{id: wasmcloud-nats, config: ...}]` and strip them from the
  workload manifest (keep subscriptions and behaviour keys), wrapped as a
  `WorkloadDeployment`. The shipped manifests deploy as-is on Cosmonic Desktop.
...
| Rust toolchain | wit-bindgen 0.60 (`async-spawn`), `wasm32-wasip1` + `wash build` |
| Go toolchain | componentize-go `main` (reports 0.4.1, vendors wit-bindgen 0.61.1) + patched Go (golang/go#76775, auto-installed) |
