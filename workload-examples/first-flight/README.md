...
The page a freshly launched WebAssembly workload serves back on your own machine, and the "you're flying" moment for [Cosmonic Desktop](https://cosmonic.com/)'s first-flight lesson. This is the source behind the **First Flight** entry in the Desktop Launchpad.
...
The component is available as an OCI artifact at `ghcr.io/cosmonic-labs/first-flight`. While it was written for Cosmonic Desktop and Cosmonic Control, it runs on any WebAssembly runtime that supports the Component Model and the [WASI](https://wasi.dev/) HTTP API.
...
[`manifests/workload.yaml`](manifests/workload.yaml) is the manifest the Launchpad
entry applies, so this is how to reproduce what the **First Flight** card
launched. Apply it and open `http://first-flight.localhost:8200`.

It runs the published image, digest-pinned:
`ghcr.io/cosmonic-labs/first-flight:0.1.2`. To run your own build instead,
`wash build`, promote it, and swap the `image` reference for the one promote
gives you.

## Deploy on Cosmonic Control

[`manifests/component.yaml`](manifests/component.yaml) is the Kubernetes path for
the same component: an `HTTPTrigger` running the published image behind an
ingress host. Apply it to a cluster running Cosmonic Control:
