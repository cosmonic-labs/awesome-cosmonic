//! postgres-mcp: a PostgreSQL MCP server for Cosmonic Desktop.
//!
...
//! specification. Its tools ([`server`]) talk to one PostgreSQL database
//! through the host's `wasmcloud:postgres@0.2.0` interface ([`postgres`]),
//! imported under the component-model labels `db` and `db-prepared`
//! (`wit/world.wit`) that Cosmonic Desktop routes to the workload's named
//! `hostInterfaces`.
//!
//! Compared with the template this crate started from, two things differ:
//! `wasip3` 0.8 / wit-bindgen 0.61 (the runtime that can express labeled
//! imports), and a second job kind in [`bridge`] — [`bridge::host_call`] —
//! that runs an arbitrary host call (a query plus its row stream) in
//! component-model context and hands the typed result to tool code.
//!
...
//! [`bridge::host_call`] for how tool code reaches the postgres host
//! interface, and [`bridge::outbound`] for outbound HTTP (unused by this
//! server's tools; kept intact).
//!
...
impl wasip3::exports::http::handler::Guest for Component {
    async fn handle(request: Request) -> Result<Response, ErrorCode> {
        ...
        if discovery::matches(&request) {
            let body = discovery::document(&server::PostgresServer::tool_names());
        ...
        // Drop jobs left over from a previous exchange that ended
        ...
        let service = StreamableHttpService::new(
            || Ok(server::PostgresServer::new()),
            Arc::new(NeverSessionManager::default()),
            config,
        );

        // Run rmcp dispatch until response headers are available. Tool code
        // runs in here; any host call (postgres) or outbound HTTP it submits
        // is serviced by the bridge along the way.
