...
pub fn document(tools: &[String]) -> String {
    ...
    let document = serde_json::json!({
        ...
        "skills": skills,
        // Layer-1 credential discovery: which secret ref this server needs
        // (presence only). Here the ref is consumed by the host's postgres
        // interface, not by the component, so a running server implies it
        // was bound; see `postgres::credentials`.
        "credentials": crate::postgres::credentials(),
