...
///
/// `skills/server/` is this server's own manual. The `building-mcp-servers`
/// skill that also lives under `skills/` is guidance for *authoring* a server
/// and is deliberately **not** listed here — it is for the developer's coding
/// agent, not for this server's clients.
pub static SKILLS: &[Skill] = &[Skill {
    ...
    files: &[
        ...
        // exactly what progressive disclosure is for.
        SkillFile {
