...
//! sequence counter, two poll timestamps, and the current client id. There is
//! exactly one After Effects instance on the other end, so there is nothing
//! to shard.
...
///
/// The default is `ae` rather than the package name on purpose: it is the
/// prefix earlier versions of this server used, so an in-flight command
/// survives the upgrade instead of being orphaned in the store.
fn key(suffix: &str) -> String {
    let prefix = std::env::var("MCP_BRIDGE_KEY_PREFIX").unwrap_or_else(|_| "ae".to_owned());
...
/// while still letting `bridge-status` tell "a stale panel is polling" apart
