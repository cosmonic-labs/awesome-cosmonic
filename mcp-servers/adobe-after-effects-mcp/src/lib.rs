...
//!
//! - the **MCP transport**, for clients — see [`server`] for the handler and
//!   [`live`] for the tools that drive After Effects;
//! - **`/bridge/*`**, polled by the MCP Bridge Auto panel running inside After
...
impl wasip3::exports::http::handler::Guest for Component {
    async fn handle(request: Request) -> Result<Response, ErrorCode> {
        ...
        // MCP exchange (a batch can hold one for minutes). Every other request
        // — including every POST — falls through to the MCP transport.
        if discovery::matches(&request) {
            let body = discovery::document(&server::AfterEffectsServer::tool_names());
...
fn transport_config() -> StreamableHttpServerConfig {
    ...
    // header, `ae-mcp.localhost.cosmonic.sh`) must set `MCP_ALLOWED_HOSTS` to
    // the comma-separated hosts to accept (`host` matches any port,
    // `host:port` is exact), or to `*` to disable the guard entirely when an
    // ingress in front of the component already pins the Host.
...
/// The After Effects ScriptUI panel, served at `/bridge/panel.jsx` so the
/// installed copy can never drift from what this component expects.
const PANEL_JSX: &str = include_str!("../bridge/mcp-bridge-auto.jsx");

/// Serves the endpoints the After Effects panel uses, or returns `None` for
...
fn serve_bridge_endpoint(
    ...
    match (&parts.method, path) {
        ...
        // Served as octet-stream, not JavaScript: the panel fetches this to
        // write it to disk, and a browser opening it should download rather
        // than try to run it.
...
///
/// No CORS headers anywhere: the ScriptUI panel talks to these routes over a
/// raw socket with no origin of its own, so nothing needs them, and a browser
/// page must not be able to read this server's state cross-origin.
fn simple_response(
    status: u16,
    content_type: &str,
...
/// Builds the discovery document's response. Same shape as
/// [`simple_response`]; named separately because the default route is part of
/// the template's contract rather than the bridge's.
fn static_response(
    status: u16,
    content_type: &str,
    body: impl Into<bytes::Bytes>,
) -> Result<Response, ErrorCode> {
    simple_response(status, content_type, body)
}
