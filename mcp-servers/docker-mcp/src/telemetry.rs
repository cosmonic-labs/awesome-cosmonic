...
pub fn init() {
    ...
    INIT.call_once(|| {
        ...
        // rmcp's service loop logs every inbound request verbatim at DEBUG
        // (`received request ... arguments: {...}`), which for this server
        // means container Env values and registry credentials would land in
        // the host log under RUST_LOG=debug. Pin that target to INFO; the
        // tool spans carry bounded identifiers only (see server.rs).
