//! docker-mcp: the Docker Engine / podman REST API as an MCP server on Cosmonic Desktop.
//!
...
impl wasip3::exports::http::handler::Guest for Component {
    async fn handle(request: Request) -> Result<Response, ErrorCode> {
        ...
        if discovery::matches(&request) {
            let body = discovery::document(&server::DockerServer::tool_names());
