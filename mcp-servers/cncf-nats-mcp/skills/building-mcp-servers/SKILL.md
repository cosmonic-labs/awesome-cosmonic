...
> **In THIS repo the entry point is `scripts/smoke.sh`, and it tests the
> deployed workload rather than a local `wasmtime serve`.** The guidance below
> still applies to its tool cases; only the harness differs. The reason is
> structural, and worth knowing if you add a host capability to any server
> built from this template: this component imports `wasmcloud:nats@0.1.0`, and
> `wasmtime serve` cannot satisfy that import, so there is no standalone
> runtime left to test against. Any server that takes a host binding beyond
> `wasi:*` inherits the same constraint.
...
(`oci.localhost:8200` image — `oci.localhost.cosmonic.sh:8200` on Windows,
which resolves no `*.localhost` names — `<name>.localhost` host) and
