...
impl wasip3::exports::http::handler::Guest for Component {
    async fn handle(request: Request) -> Result<Response, ErrorCode> {
        ...
        // MCP exchange (a diagnostic tool can hold one for its whole sampling
        // window). Every other request — including every POST — falls through
        // to the MCP transport unchanged.
        if discovery::matches(&request) {
            let body = discovery::document(&server::NatsServer::tool_names());
...
fn transport_config() -> StreamableHttpServerConfig {
    ...
    // header, `nats-mcp-server-v1.localhost`) must set `MCP_ALLOWED_HOSTS` to the
...
/// Builds a complete, non-streamed response — used for the discovery document,
/// which is small, known in full up front, and must not touch the transport.
