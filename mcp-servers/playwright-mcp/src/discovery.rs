...
pub fn document(tools: &[String]) -> String {
    ...
    let document = serde_json::json!({
        ...
        // Layer-1 credential discovery (presence only, never values): the
        // optional bearer token for a hosted upstream, and whether it is set.
        "credentials": crate::playwright::credentials(),
        "capabilities": {
            // The local tools plus whatever upstream tools are cached on
            // this instance (the full browser tool set appears once the
            // upstream has answered one tools/list).
            "tools": tools,
            "resources": {
